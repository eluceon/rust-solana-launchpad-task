...
6. **Тесты** (LiteSVM, без сети):
   ...
   под glibc >= 2.38 и не загружается на Ubuntu 22.04. Node запускается с флагом
   `--no-allocation-site-pretenuring`: нативный модуль litesvm провоцирует спорадический
   `std::bad_alloc` в GC V8 (rehash таблицы AllocationSite в pretenuring feedback),
   флаг отключает этот механизм и делает прогоны детерминированными.
