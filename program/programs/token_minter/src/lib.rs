...
declare_id!("7wzZRUV2jvg5fyecgdaDVXBa3eYP7yJ2KTqBCLjxPKTa");
...
fn compute_fee_lamports(mint_fee_usd: u64, price: u64) -> Result<u64> {
    ...
    // Both mint_fee_usd and price use 6 decimals, so they cancel out and the
    // result is denominated in lamports. Widen to u128 to avoid overflow in
    ...
    u64::try_from(fee).map_err(|_| error!(MinterError::MathOverflow))
}
